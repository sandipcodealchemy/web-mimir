...
export default function Students() {
  ...
  const [editValue, setEditValue] = useState<Student | null>(null)
  ...
  const _onAdd = async () => {
    ...
    Api.post(API_CONSTANT.STUDENTS, data)
      .then((res) => {
        setTimeout(() => {
          setLoading(false)
        }, 1000)
        ...
        _studentListAPI()
        setOpen(false)
        toast.success(MSG.ADD_NEW_STUDENT)
      })
      .catch((error) => {
        setLoading(false)
        toast.error(error?.response?.data?.error)
  ...
  const _onEditDeleteOpen = (val: Student, type: string) => {
    setOpen(true)
    setEditValue(val)
    setName(val.name)
    setType(type)
  ...
  const _onEdit = () => {
    ...
    Api.patch(API_CONSTANT.STUDENTS, data)
      .then((res) => {
        setOpen(false)
        _clearInputs()
        _studentListAPI()
        toast.success(MSG.EDIT_STUDENT)
      })
      .catch((error) => {
        toast.error(error?.response?.data?.error)
      })
  ...
  const _onDelete = () => {
    ...
    Api.patch(API_CONSTANT.STUDENTS, data)
      .then((res) => {
        ...
        _clearInputs()
        _studentListAPI()
        toast.success(MSG.DELETE_STUDENT)
      })
      .catch((error) => {
        toast.error(error?.response?.data?.error)
      })
        ...
        <div className="mt-[97px] flex">
          ...
          >
            {loading ? (
              ...
              <p>{AppConstant.ADD_NEW_STUDENT}</p>
            )}
          </button>
        </div>
      </div>
          ...
          <div className="flex">
            <p className="text_MB_16 w-full text-Ellipse2003">
              {type === AppConstant.EDIT
                ? AppConstant.ARE_YOU_SURE_STUDENT_EDIT
                : type === AppConstant.DELETE
                ? AppConstant.ARE_YOU_SURE_STUDENT_DELETE
                : AppConstant.STUDENT_NAME}
            </p>
