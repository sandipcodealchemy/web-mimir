...
export default function Classes() {
  ...
  const [editValue, setEditValue] = useState<Class | null>(null)
  ...
  const _onAdd = async () => {
    ...
    Api.post(API_CONSTANT.CLASSES, data)
      .then((res) => {
        setTimeout(() => {
          setLoading(false)
        }, 1000)
        ...
        _classListAPI()
        setOpen(false)
        toast.success(MSG.ADD_NEW_CLASS)
      })
      .catch((error) => {
        setLoading(false)
        toast.error(error?.response?.data?.error)
  ...
  const _onEditDeleteOpen = (val: Class, type: string) => {
    setOpen(true)
    setEditValue(val)
    setName(val.name)
    setType(type)
  ...
  const _onEdit = () => {
    ...
    Api.patch(API_CONSTANT.CLASSES, data)
      .then((res) => {
        setOpen(false)
        _clearInputs()
        _classListAPI()
        toast.success(MSG.EDIT_CLASS)
      })
      .catch((error) => {
        toast.error(error?.response?.data?.error)
      })
  ...
  const _onDelete = () => {
    ...
    Api.patch(API_CONSTANT.CLASSES, data)
      .then((res) => {
        ...
        _clearInputs()
        _classListAPI()
        toast.success(MSG.DELETE_CLASS)
      })
      .catch((error) => {
        toast.error(error?.response?.data?.error)
      })
        ...
        <div className="mt-[97px] flex">
          ...
          >
            {loading ? (
              ...
              <p className="">{AppConstant.ADD_NEW_CLASS}</p>
            )}
          </button>
        </div>
      </div>
          ...
          <div className="flex">
            <p className="text_MB_16 w-full text-Ellipse2003">
              {type === AppConstant.EDIT
                ? AppConstant.AUR_YOU_SURE_CLASS_EDIT
                : type === AppConstant.DELETE
                ? AppConstant.AUR_YOU_SURE_CLASS_DELETE
                : AppConstant.CLASS_NAME}
            </p>
